    ...
    <div className="space-y-4 animate-fadeIn">
      ...
      <Card className="flex flex-col md:flex-row items-start md:items-center justify-between gap-4">
        ...
        <div className="flex items-center gap-3">
          <div className="px-3 py-1.5 rounded-lg bg-surface-900 border border-slate-800 text-right">
            <span className="text-[11px] text-slate-400 block">Current Clock</span>
          ...
          <div className="px-3 py-1.5 rounded-lg bg-surface-900 border border-slate-800 text-right">
            <span className="text-[11px] text-slate-400 block">Overall Load</span>
      ...
      <Card className="space-y-3">
        <div className="flex justify-between items-center text-xs">
          <div className="flex items-center gap-2">
            ...
            <h3 className="font-bold text-slate-200">Per-Core Activity & Utilization</h3>
          </div>
          <span className="text-slate-400 font-mono">{cores.length} Active Threads</span>
        </div>
        ...
        <div className="grid grid-cols-2 sm:grid-cols-3 md:grid-cols-4 lg:grid-cols-6 gap-2.5">
          {cores.map((usage, idx) => (
            <div
              key={idx}
              className="p-2.5 rounded-lg bg-surface-900 border border-slate-800/80 flex flex-col justify-between gap-1.5"
            >
              <div className="flex justify-between items-center text-[11px]">
                <span className="text-slate-400 font-medium">Core #{idx}</span>
                <span className="font-mono font-bold text-slate-200">{Math.round(usage)}%</span>
              </div>
      ...
      <Card className="p-4 bg-surface-900/60 border-slate-800 flex items-start gap-3 text-xs text-slate-400">
        <Info className="w-4 h-4 text-brand-400 flex-shrink-0 mt-0.5" />
        <div className="space-y-1">
          <p className="text-slate-200 font-medium">Windows CPU Scheduling & Power Profiles</p>
