      ...
      <div className="flex items-center justify-between">
        <div>
          <h2 className="text-base font-bold text-slate-900 dark:text-slate-100 flex items-center gap-2">
            <CircuitBoard className="w-5 h-5 text-indigo-500 dark:text-indigo-400" /> Hardware Specification & Diagnostics
          </h2>
          <p className="text-xs text-slate-500 dark:text-slate-400">Motherboard, BIOS firmware, CPU topology, and expansion slots</p>
        </div>
      </div>
        ...
        <Card className="space-y-3">
          ...
          <div className="space-y-2 text-xs">
            <div className="flex justify-between py-1 border-b border-slate-200 dark:border-slate-800/50">
              ...
              <span className="font-semibold text-slate-900 dark:text-slate-200 truncate max-w-xs">{stats?.cpu_name || 'x86_64 Processor'}</span>
            </div>
            <div className="flex justify-between py-1 border-b border-slate-200 dark:border-slate-800/50">
              <span className="text-slate-500 dark:text-slate-400">Instruction Set:</span>
              <span className="font-semibold font-mono text-slate-900 dark:text-slate-200">x86-64 / AMD64 (AVX2, SSE4.2)</span>
            </div>
            <div className="flex justify-between py-1 border-b border-slate-200 dark:border-slate-800/50">
              <span className="text-slate-500 dark:text-slate-400">Logical Cores:</span>
              <span className="font-semibold font-mono text-brand-600 dark:text-brand-300">{stats?.cpu_cores?.length || 8} Threads</span>
            </div>
            <div className="flex justify-between py-1">
              <span className="text-slate-500 dark:text-slate-400">Total System Memory:</span>
              <span className="font-semibold font-mono text-slate-900 dark:text-slate-200">{formatBytes(stats?.ram_total_bytes || 0)}</span>
            </div>
          </div>
        </Card>
      </div>
