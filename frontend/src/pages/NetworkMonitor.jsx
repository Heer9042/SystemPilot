        ...
        <Card className="space-y-2">
          <div className="flex items-center justify-between">
            <div className="flex items-center gap-2">
              ...
              <h3 className="text-sm font-bold text-slate-200">Real-time Download Throughput</h3>
            </div>
        ...
        <Card className="space-y-2">
          <div className="flex items-center justify-between">
            <div className="flex items-center gap-2">
              ...
              <h3 className="text-sm font-bold text-slate-200">Real-time Upload Throughput</h3>
            </div>
      ...
      <Card className="space-y-3">
        <div className="flex items-center justify-between pb-2 border-b border-slate-800">
          <div className="flex items-center gap-2">
            ...
            <h3 className="text-sm font-bold text-slate-100">Network Interfaces & IP Configuration</h3>
          </div>
        </div>
          ...
          {interfaces.map((iface, idx) => (
            <div key={idx} className="py-3 flex flex-col sm:flex-row items-start sm:items-center justify-between gap-2">
              <div className="space-y-1">
                <div className="flex items-center gap-2">
                  <span className="font-medium text-slate-200 text-xs">{iface.name}</span>
              ...
              <div className="text-right text-xs font-mono">
                <div className="text-slate-300">↓ {formatBytes(iface.total_received_bytes)}</div>
                <div className="text-slate-400">↑ {formatBytes(iface.total_transmitted_bytes)}</div>
              </div>
            </div>
          ))}
        </div>
