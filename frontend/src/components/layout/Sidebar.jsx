...
export function Sidebar({ activeTab, setActiveTab, onQuickClean, ramUsagePercent }) {
  ...
  return (
    <aside
      className={`${
        collapsed ? 'w-16' : 'w-60'
      } h-full bg-surface-950/95 border-r border-slate-800/80 flex flex-col justify-between select-none z-20 backdrop-blur-xl transition-all duration-200 shrink-0`}
    >
      {/* Brand Header */}
      <div className="p-3 border-b border-slate-800/60 flex items-center justify-between">
          ...
          {!collapsed && (
            <div className="overflow-hidden">
              <div className="flex items-center gap-1.5">
                <h1 className="font-bold text-slate-100 text-sm tracking-tight truncate">SystemPilot</h1>
                <span className="text-[9px] font-bold px-1.5 py-0.2 bg-brand-500/20 text-brand-300 border border-brand-500/30 rounded">
                  v0.0.2
                </span>
              </div>
              <p className="text-[10px] text-slate-400 truncate">System Suite</p>
            </div>
          )}
        </div>
        ...
        <button
          ...
          className="p-1 rounded-md text-slate-400 hover:text-slate-200 hover:bg-surface-800 transition shrink-0"
        >
          ...
          return (
            <button
              ...
              className={`w-full flex items-center ${
                ...
                isActive
                  ? 'bg-brand-600/20 text-brand-300 border border-brand-500/30 shadow-sm shadow-brand-500/10 font-semibold'
            ...
            >
              ...
              {!collapsed && item.badge && (
                <span className="text-[9px] font-semibold px-1.5 py-0.2 rounded bg-brand-500/30 text-brand-300 shrink-0">
                  {item.badge}
                </span>
              )}
            </button>
      ...
      <div className="p-2 border-t border-slate-800/60 bg-surface-900/40">
        {!collapsed ? (
          <div className="glass-panel p-2 rounded-lg border-slate-800/80 flex flex-col gap-1.5">
            <div className="flex justify-between items-center text-xs">
              <span className="text-slate-400 text-[11px]">RAM Load</span>
              <span className="font-mono font-bold text-brand-300 text-[11px]">
          ...
          <button
            ...
            className="w-full p-2 rounded-lg bg-brand-600/20 hover:bg-brand-600/40 text-brand-400 flex items-center justify-center transition"
          >
