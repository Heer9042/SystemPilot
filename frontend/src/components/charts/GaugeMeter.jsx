    ...
    <div className="flex flex-col items-center justify-center p-3 relative">
      <div className="relative w-28 h-28 flex items-center justify-center">
        ...
        <div className="absolute flex flex-col items-center justify-center">
          {Icon && <Icon className="w-4 h-4 text-slate-400 mb-0.5" />}
      ...
      {label && <span className="text-xs font-semibold text-slate-200 mt-1">{label}</span>}
...
export function MemoryBreakdownBar({
  ...
  return (
    <div className="w-full flex flex-col gap-2">
      <div className="h-6 w-full rounded-xl overflow-hidden flex bg-surface-900 border border-slate-700/40 p-0.5">
        <div
      ...
      <div className="flex flex-wrap items-center justify-between text-xs pt-1">
        <div className="flex items-center gap-2">
          ...
          <span className="font-semibold text-slate-200">{formatBytesFn ? formatBytesFn(used) : `${usedPct.toFixed(1)}%`}</span>
        </div>
        <div className="flex items-center gap-2">
          <span className="w-2.5 h-2.5 rounded-full bg-amber-500" />
          <span className="text-slate-400">Standby / Cached:</span>
          <span className="font-semibold text-slate-200">{formatBytesFn ? formatBytesFn(standby) : `${standbyPct.toFixed(1)}%`}</span>
        </div>
        <div className="flex items-center gap-2">
          <span className="w-2.5 h-2.5 rounded-full bg-emerald-500" />
          <span className="text-slate-400">Free:</span>
          <span className="font-semibold text-slate-200">{formatBytesFn ? formatBytesFn(free) : `${freePct.toFixed(1)}%`}</span>
        </div>
      </div>
    </div>
  );
