  ...
  return (
    <div className="space-y-5 animate-fadeIn">
      ...
      <div className="grid grid-cols-1 lg:grid-cols-4 gap-4">
        <Card className="lg:col-span-3 flex flex-col md:flex-row items-start md:items-center justify-between gap-4 bg-gradient-to-r from-surface-900 via-surface-900 to-brand-950/40 border-brand-500/20">
          <div className="space-y-1">
            <div className="flex items-center gap-2">
              <span className="h-2 w-2 rounded-full bg-emerald-400 animate-ping" />
              <h2 className="text-lg font-bold text-slate-100">{stats.host_name}</h2>
              <span className="text-xs px-2 py-0.5 rounded-full bg-surface-800 text-slate-300 border border-slate-700">
                {stats.os_name}
              </span>
            </div>
        ...
        <Card className="flex flex-col justify-between">
          ...
          <div className="my-2">
            ...
            <p className="text-[11px] text-slate-400 flex items-center gap-1 mt-1">
              <Clock className="w-3 h-3 text-slate-500" /> System Uptime
            </p>
          </div>
          <div className="text-xs text-slate-300 flex items-center justify-between border-t border-slate-800 pt-2">
            <span>Processes:</span>
            <span className="font-bold font-mono text-brand-300">{stats.process_count}</span>
          </div>
        </Card>
      </div>
      ...
      <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
        ...
        <Card hover onClick={() => setActiveTab('network')} className="flex flex-col items-center justify-center p-4">
          <div className="w-full flex flex-col gap-3">
            <div className="flex items-center justify-between text-xs text-slate-400">
              <span className="flex items-center gap-1.5"><Wifi className="w-4 h-4 text-sky-400" /> Network Activity</span>
            </div>
            <div className="space-y-2">
              <div className="flex items-center justify-between bg-surface-900/80 p-2 rounded-lg border border-slate-800">
        ...
        <Card className="flex flex-col gap-2">
          <div className="flex items-center justify-between">
            <div className="flex items-center gap-2">
              ...
              <h3 className="text-sm font-bold text-slate-200">CPU Usage Timeline</h3>
            </div>
            <span className="font-mono text-xs text-indigo-400 font-bold">{Math.round(stats.cpu_usage)}%</span>
          </div>
        ...
        <Card className="flex flex-col gap-2">
          <div className="flex items-center justify-between">
            <div className="flex items-center gap-2">
              ...
              <h3 className="text-sm font-bold text-slate-200">RAM Usage Timeline</h3>
            </div>
            <span className="font-mono text-xs text-cyan-400 font-bold">{Math.round(stats.ram_usage_percent)}%</span>
          </div>
          <AreaChartLive data={history.ram} color="#06b6d4" unit="%" height={110} />
        </Card>
      </div>

      {/* Battery & Power Overview (for laptops and desktops) */}
      <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
        <Card className="flex items-center gap-3.5">
          <div className="p-3 rounded-xl bg-surface-800 text-amber-400 border border-slate-700/50">
            {stats.is_charging ? <BatteryCharging className="w-6 h-6" /> : <Battery className="w-6 h-6" />}
          </div>
          <div>
            <span className="text-xs text-slate-400">Power & Battery</span>
        ...
        <Card className="flex items-center gap-3.5">
          <div className="p-3 rounded-xl bg-surface-800 text-indigo-400 border border-slate-700/50">
            <Server className="w-6 h-6" />
          </div>
          <div>
            <span className="text-xs text-slate-400">Virtual / Swap Memory</span>
            <div className="text-base font-bold text-slate-100">
        ...
        <Card className="flex items-center gap-3.5">
          <div className="p-3 rounded-xl bg-surface-800 text-emerald-400 border border-slate-700/50">
            <Activity className="w-6 h-6" />
          </div>
          <div>
            <span className="text-xs text-slate-400">Total Network Data</span>
